<template>
	<NewModal ref="modal" :header="formatMessage(messages.header)" max-width="544px" no-padding>
		...
		<div class="flex flex-col gap-6 px-6 pb-6">
			<div class="flex flex-col gap-2">
				<Button type="colored" color="brand" @click="openOfflineModal">
				...
				<div class="grid grid-cols-2 gap-2 mt-1">
					...
					<Button type="quiet" :disabled="loadingSignIn" @click="signIn">
	...
	<OfflineAccountModal ref="offlineModal" @created="onOfflineAccountCreated" />
</template>

<script setup lang="ts">
import { MessagesSquareIcon, SpinnerIcon, UserIcon } from '@freeplay/assets'
import { Button, ButtonLink, defineMessages, NewModal, useVIntl } from '@freeplay/ui'
...
const messages = defineMessages({
	header: {
		...
		defaultMessage: 'Minecraft account profile',
	},
	descriptionHeader: {
	...
	description: {
		...
		defaultMessage:
			'Play instantly for free with an offline player profile, or sign in with your official Microsoft account.',
	},
	getSupport: {
	...
	dontHaveAccount: {
		...
		defaultMessage: 'Want an official account?',
	},
	getMinecraft: {
...
async function signIn() {
	...
	const loggedIn = await loginFlow().catch(handleSevereError)
