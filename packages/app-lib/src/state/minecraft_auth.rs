...
pub async fn create_offline_account(
    ...
    if clean_username.is_empty() || clean_username.len() > 16 || clean_username.contains(' ') {
        return Err(crate::ErrorKind::OtherError("Invalid Minecraft username. Must be 1-16 characters without spaces.".to_string()).into());
    }

    let offline_uuid = freeplay_domain::generate_offline_uuid(&clean_username);
...
pub const MINECRAFT_SERVICES_USER_AGENT: &str =
    "FreePlay/1.0.0 (support@freeplay.app)";
