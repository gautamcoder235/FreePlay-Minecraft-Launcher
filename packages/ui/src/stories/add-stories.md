...
This document provides instructions for AI assistants when creating Storybook stories for Vue components in the `@freeplay/ui` package.
...
Import icons from `@freeplay/assets`:

```typescript
import { SearchIcon, ChevronDownIcon } from '@freeplay/assets'
...
### 1. Don't Import from `@freeplay/ui` in Components
