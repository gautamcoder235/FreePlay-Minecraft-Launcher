...
import { injectFreePlayClient, provideFreePlayServerContext } from '../providers'
import type { BusyReason, CancelUploadHandler, ServerStats } from '../providers/server-context'
import { defineMessage } from './i18n'
import { useFreePlayServersConsole } from './server-console'
import {
...
export function useServerManageCoreRuntime(options: UseServerManageCoreRuntimeOptions) {
	...
	const handleLog = (data: Archon.Websocket.v0.WSLogEvent) => {
		if (!shouldProcessEvent()) return
		freeplayServersConsole.recordWsEvent({ event: 'log', ...data })
		freeplayServersConsole.addLegacyLog(data.message)
	}

	const handleLog4j = (data: Archon.Websocket.v0.WSLog4jEvent) => {
		if (!shouldProcessEvent()) return
		freeplayServersConsole.recordWsEvent({ event: 'log4j', ...data })
		freeplayServersConsole.addLog4jEvent(data)
	}
	...
	const connectSocket = async (
		...
		try {
			...
			freeplayServersConsole.beginInitialLogHydration()
